...
res = morphometricity.morph_fit(y=y, X=X, K=K, method="expected", max_iter=100, tol=10**(-4))
res['flag']+' in '+str(res['iteration'])+' iterations'
res['Estimated morphometricity']
# add simulation for non positive semi definite ASM
# %%
# linear ASM
...
res = morphometricity.morph_fit(y=y, X=X, K=K, method="observed", max_iter=500, tol=10**(-4))
res['flag']+' in '+str(res['iteration'])+' iterations'
